...
function Invoke-Install {
  ...
  # ลองดาวน์โหลดเวอร์ชันล่าสุดก่อนเสมอ แล้วค่อยถอยไปใช้ไฟล์ในเครื่องถ้าโหลดไม่ได้
  #
  # เดิมทำกลับกัน คือถ้ามีไฟล์ข้างตัวช่วยติดตั้งจะใช้ไฟล์นั้นทันที ซึ่งทำให้เครื่องที่
  # ต่อเน็ตได้ติดอยู่กับไฟล์เก่าถาวร กดเมนู 1 กี่ครั้งก็ได้เวอร์ชันเดิม โดยไม่มีอะไร
  # บอกผู้ใช้เลยว่าทำไมไม่ได้ตัวใหม่
  $got = $false
  if ($local) {
    Step 'พบไฟล์โปรแกรมที่วางไว้ข้างตัวช่วยติดตั้ง - จะลองเอาเวอร์ชันล่าสุดก่อน'
    $got = Get-RemoteFile $APP_URL $zip 'ตัวโปรแกรมเวอร์ชันล่าสุด'
    if (-not $got) {
      Warn 'ดาวน์โหลดไม่ได้ - จะใช้ไฟล์ที่วางไว้ในเครื่องแทน'
      Warn 'ไฟล์นี้อาจไม่ใช่เวอร์ชันล่าสุด ถ้าต้องการตัวใหม่ให้ลบ ndp-kit.zip ทิ้งแล้วลองใหม่ตอนเน็ตใช้ได้'
