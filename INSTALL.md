...
3. ดู **รหัสสำหรับตั้งค่าครั้งแรก** ที่โปรแกรมสุ่มให้:

   ```bash
   docker compose logs | grep "รหัสสำหรับตั้งค่าครั้งแรก"
   ```

   รหัสนี้ป้องกันไม่ให้คนอื่นในวง LAN ชิงเข้าหน้าตั้งค่าก่อนแล้วชี้ฐานข้อมูลไปเครื่องของตัวเอง
   ถ้าอยากกำหนดเองแทนการสุ่ม ให้ใส่ `SETUP_TOKEN=รหัสที่ต้องการ` ในไฟล์ `.env` ข้างไฟล์ compose

4. เปิดเบราว์เซอร์ไปที่ `http://localhost:3000` (หรือ `http://<IP เครื่องนี้>:3000` จากเครื่องอื่นในวง LAN) กรอกรหัสจากข้อ 3 แล้วตั้งค่าฐานข้อมูลผ่านหน้า `/settings` ตามขั้นตอนที่ 7 — ค่าตั้งจะถูกเก็บในโฟลเดอร์ `./data` ข้างไฟล์ compose (mount เป็น volume) จึง**ไม่หายเวลาอัปเดต**

   เมื่อบันทึกการตั้งค่าสำเร็จ รหัสนี้จะใช้ไม่ได้อีก ต่อจากนั้นการเข้าหน้าตั้งค่าต้องเข้าสู่ระบบด้วยบัญชีเจ้าหน้าที่เสมอ
